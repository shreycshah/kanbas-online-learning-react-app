...
export default function QuizDetails() {
  ...
  function questionAndAnswer() {
    ...
    setMarkedAnswers(
      actual.map((question: any) => {
        let res = given.filter((response: any) => response.questionId == question._id);
        console.log( res[0])
        ...
        return { ...question, markedAnswer: res[0].answer,points:points, answer:String(question.answer) };
      })
    );
    }
  ...
  return (
    <div className="container mt-4">
      ...
      <br />
      {currentUser && currentUser?.role == "STUDENT" && <h3>Marked Answers:</h3>}
      <div>
        <div className="row ">
          <div className="col-1"></div>
          <div className="col-10">
            {markedAnswers && markedAnswers.map((markedAnswer,index)=>{
                return <div className="align-items-center border border-dark rounded-1 mb-5">
                ...
                <div>
                  <div>
                    {markedAnswer.type == "tf" && ["true","false"].map((option)=>{
                        return <div>
                            <hr/>
                        ...
                        <div className="col-3">
                          {" "}
                          <AnswerTypeElement
                            text={markedAnswer.markedAnswer == option ? option == markedAnswer.answer?"Correct!":"You Answered":option == markedAnswer.answer?"Correct Answer":""}
                            color={markedAnswer.markedAnswer == option ? option == markedAnswer.answer?"btn-success":"btn-danger":option == markedAnswer.answer?"btn-secondary":""}
                          />{" "}
                        </div>
                    ...
                    {markedAnswer.type == "mcq" && markedAnswer.choices.map((option:any)=>{
                        return <div>
                            <hr/>
                        <div className="d-flex align-items-center ">
                        {/* <div className=" fw-bold text-white bg-success d-flex align-items-center border border-dark rounded-1 p-2 ps-3 pe-3">Correct !</div>  */}
                        <div className="col-3">
                          {" "}
                          <AnswerTypeElement
                            text={markedAnswer.markedAnswer == option.answer ? option.isCorrect?"Correct!":"You Answered":option.isCorrect?"Correct Answer":""}
                            color={markedAnswer.markedAnswer == option.answer ? option.isCorrect?"btn-success":"btn-danger":option.isCorrect?"btn-secondary":""}
                          />{" "}
                        </div>
                        <div className="col-9 p-2">
                          {" "}
                    ...
                    {markedAnswer.type == "fib" && <MarkedAnswerFillInTheBlanks markedAnswer = {markedAnswer}/>}
                  </div>
                </div>
              </div>
            })}
